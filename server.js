...
function isAdminCode(code){return ADMIN_CODE && String(code||'')===String(ADMIN_CODE)}
function cleanImage(image){
  const v=String(image||'').trim();
  if(!v) return 'assets/logo.png';
  if(v.startsWith('data:image/')) return v.slice(0,700000);
...
app.get('/api/admin/ctvs',(req,res)=>{if(!isAdminCode(req.headers['x-admin-code']))return res.status(401).json({error:'Mã quản trị không đúng'});res.json(ctvs.map(c=>({code:c.code,name:c.name,platform:c.platform||'Chưa rõ',bound:!!c.deviceId,lastSeen:c.lastSeen||null})))});
app.post('/api/admin/product',(req,res)=>{if(!isAdminCode(req.headers['x-admin-code']))return res.status(401).json({error:'Mã quản trị không đúng'});const {action,id,name,price,image,category}=req.body;if(action==='delete'){products=products.filter(p=>p.id!==id);db.save('products.json',products);return res.json({ok:true})}if(action==='update'){const p=products.find(x=>x.id===id);if(!p)return res.status(404).json({error:'Không tìm thấy sản phẩm'});if(name!=null)p.name=String(name).slice(0,120);if(price!=null){const n=Number(price);if(!n)return res.status(400).json({error:'Giá không hợp lệ'});p.price=n}if(image!=null)p.image=cleanImage(image);if(category)p.category=String(category);db.save('products.json',products);return res.json({ok:true,product:p})}if(action==='add'){if(!name||!Number(price))return res.status(400).json({error:'Thiếu tên/giá'});const p={id:crypto.randomBytes(3).toString('hex'),name:String(name).slice(0,120),price:Number(price),image:cleanImage(image),category:['nu','nam','unisex'].includes(category)?category:'unisex',createdAt:Date.now()};products.push(p);db.save('products.json',products);return res.json({ok:true,product:p})}res.status(400).json({error:'Action không hợp lệ'})});
